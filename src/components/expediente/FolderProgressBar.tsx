    ...
    <div className="bg-surface-container-lowest border border-outline-variant rounded-lg p-5 shadow-institutional">
      <div className="flex justify-between items-center relative">
        ...
        {carpetas.map((folder, index) => {
          const num = (index + 1) as 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8;
          const isActive = num === activeNumero;
          const isCompleted = folder.estado === "Completado" || folder.documentos.length > 0;
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col items-center">
                ...
                <span className="text-[9px] font-mono text-outline-variant group-hover:text-primary transition-colors flex items-center gap-0.5">
                  {isAI ? <Sparkles className="w-2.5 h-2.5 text-secondary" /> : null}
                  {isAI ? "IA" : "Manual"}
                </span>
              </div>
            </button>
